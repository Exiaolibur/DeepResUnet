...
arch_names = list(unet.__dict__.keys()) #网络模型
loss_names = list(losses.__dict__.keys()) #损失函数
...
def parse_args():
    ...
    #add添加程序称述信息    其中default表示当参数未在命令行出现时使用的值
    #help表示用来描述这个选项的作用  metavar - 在使用方法消息中使用的参数值示例   choices - 可用的参数的容器。
    #type - 命令行参数应当被转换成的类型
    ...
    parser.add_argument('--deepsupervision', default=False, type=str2bool)  #默认设置
    parser.add_argument('--dataset', default="jiu0Monkey",
                        help='dataset name')
    parser.add_argument('--input-channels', default=4, type=int,  help='input channels')  #输入通道，有四路，因为有四个标签
    parser.add_argument('--image-ext', default='png', help='image file extension')  #图像文件扩展名
    ...
    parser.add_argument('--weight-decay', default=1e-4, type=float,help='weight decay')  #权重衰减
...
def train(args, train_loader, model, criterion, optimizer, epoch, scheduler=None):  #criterion标准
    losses = AverageMeter()
    ious = AverageMeter()
...
def main():
    ...
    # Data loading code  输入数据的路径
    img_paths = glob(r'E:\BRATS\data\2-MICCAI_BraTS_2018\trainImage\*')
    mask_paths = glob(r'E:\BRATS\data\2-MICCAI_BraTS_2018\trainMask\*')
